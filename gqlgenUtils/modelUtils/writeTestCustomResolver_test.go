...
func TestWriteTestCustomResolvers(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			req: args{
				...
				resolverFiles:  []string{"customCreateMutation.test.js", "customUpdateMutation.test.js", "customDeleteMutation.test.js"},
		...
		},
		{
			...
			req: args{
				...
				fields:         []string{"field"},
				fieldTypes:     []string{"fieldType"},
				resolverFiles:  []string{"customCreateMutation.test.js"},
				nullFields:     []bool{true, false},
				customMutation: true,
			},
			createErr: true,
		},
		{
			...
			req: args{
				...
				fields:         []string{"field"},
				fieldTypes:     []string{"fieldType"},
				resolverFiles:  []string{"customUpdateMutation.test.js"},
				nullFields:     []bool{true, false},
				customMutation: true,
			},
			updateErr: true,
		},
		{
			...
			req: args{
				...
				fields:         []string{"field"},
				fieldTypes:     []string{"fieldType"},
				resolverFiles:  []string{"customDeleteMutation.test.js"},
				nullFields:     []bool{true, false},
				customMutation: true,
			},
			deleteErr: true,
	...
	for _, tt := range cases {
		...
		patchCreateSource := gomonkey.ApplyFunc(
			hbs.CustomCreateMutationTestSource,
		...
		patchUpdateSource := gomonkey.ApplyFunc(
			hbs.CustomUpdateMutationTestSource,
			func(string, string, string, map[string]interface{}) error {
				if tt.updateErr {
		...
		patchDeleteSource := gomonkey.ApplyFunc(
			hbs.CustomDeleteMutationTestSource,
			func(string, string, string, map[string]interface{}) error {
				if tt.deleteErr {
		...
		t.Run(tt.name, func(t *testing.T) {
			err := modelUtils.WriteTestCustomResolvers(
				tt.req.modelName,
				tt.req.dirName,
				tt.req.fields,
				tt.req.fieldTypes,
